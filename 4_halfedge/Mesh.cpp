...
vector<vector<Halfedge*>> Mesh::Boundaries()
{
	...
	for (auto e : halfedges) {
		if (e->face == NULL and traversed_edges[e->index] == false) {
...
void Mesh::ParseFaceString(string str)
{
	...
	while (ss >> tmp) {
		if (stringstream(tmp) >> vert_idx) {
			assert(vert_idx > 0 and vert_idx <= vertices.size());
			vert_idx -= 1;
...
{
	assert(v_source->index >= 0 and v_source->index < vertices.size());
	assert(v_target->index >= 0 and v_target->index < vertices.size());
	cout << "--------------------------------" << endl;
